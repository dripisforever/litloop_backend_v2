import logging
import json
from django.conf import settings
from django.http import JsonResponse
from django.views.decorators.csrf import csrf_exempt
from tracks.models import Track
from artists.models import Artist
from users.auth_utils import jwt_required_testable
from chats.r2_utils import get_r2_client, r2_generate_presigned_url
from posts.helpers import produce_friendly_token
from tracks.upload.mutagen.mutagen_utils import extract_track_metadata

logger = logging.getLogger(__name__)


@csrf_exempt
@jwt_required_testable
def r2_initiate_track_upload(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'Only POST allowed'}, status=405)

    try:
        data = json.loads(request.body)
    except json.JSONDecodeError:
        return JsonResponse({'error': 'Invalid JSON'}, status=400)

    filename = str(data.get("filename", "unknown"))
    content_type = str(data.get("content_type", "audio/mpeg"))
    key = f"track/{filename}"

    client = get_r2_client()
    try:
        response = client.create_multipart_upload(
            Bucket=settings.R2_BUCKET_NAME,
            Key=key,
            ContentType=content_type
        )
        return JsonResponse({"upload_id": response["UploadId"], "key": key})
    except Exception as e:
        logger.exception("R2 Track Initiate Failed")
        return JsonResponse({"error": str(e)}, status=500)


@csrf_exempt
@jwt_required_testable
def r2_get_track_part_url(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'Only POST allowed'}, status=405)

    try:
        data = json.loads(request.body)
    except json.JSONDecodeError:
        return JsonResponse({'error': 'Invalid JSON'}, status=400)

    key = data.get("key")
    upload_id = data.get("upload_id")
    part_number = int(data.get("part_number", 1))

    client = get_r2_client()
    try:
        url = client.generate_presigned_url(
            ClientMethod="upload_part",
            Params={
                "Bucket": settings.R2_BUCKET_NAME,
                "Key": key,
                "UploadId": upload_id,
                "PartNumber": part_number,
            },
            ExpiresIn=3600,
        )
        return JsonResponse({"url": url})
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=500)


@csrf_exempt
@jwt_required_testable
def r2_complete_track_upload(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'Only POST allowed'}, status=405)

    try:
        data = json.loads(request.body)
    except json.JSONDecodeError:
        return JsonResponse({'error': 'Invalid JSON'}, status=400)

    key = data.get("key")
    upload_id = data.get("upload_id")
    parts = data.get("parts")

    client = get_r2_client()
    try:
        client.complete_multipart_upload(
            Bucket=settings.R2_BUCKET_NAME,
            Key=key,
            UploadId=upload_id,
            MultipartUpload={"Parts": parts}
        )
        filename = key.split("/")[-1]
        meta = extract_track_metadata(key)
        name = meta["name"] or filename
        track_uri = produce_friendly_token()
        obj = Track.objects.create(
            filename=filename, s3_key=key, status='draft',
            name=name, track_uri=track_uri, user=getattr(request, 'user', None)
        )
        if meta["artist"]:
            artist, _ = Artist.objects.get_or_create(name=meta["artist"], artist_uri=track_uri)
            obj.artists.add(artist)
        return JsonResponse({
            "status": "completed",
            "id": obj.id,
            "name": name,
            "artist": meta["artist"],
            "location": r2_generate_presigned_url(key, method="GET", expiration=604800)
        })
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=500)
