...
@csrf_exempt
@jwt_required
def r2_initiate_video_upload(request):
    ...
    try:
        ...
        logger.exception("R2 Video Initiate Failed")
        return JsonResponse({"error": str(e)}, status=500)


@csrf_exempt
@jwt_required
def r2_get_video_part_url(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'Only POST allowed'}, status=405)
...
@csrf_exempt
@jwt_required
def r2_complete_video_upload(request):
    if request.method != 'POST':
        return JsonResponse({'error': 'Only POST allowed'}, status=405)
    ...
    try:
        ...
        obj = Video.objects.create(filename=filename, s3_key=key, status='draft', user=request.user)
        return JsonResponse({
            "status": "completed",
            "id": obj.id,
