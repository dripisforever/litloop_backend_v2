...
def transcode_video(video_id):
    video_file = Video.objects.get(id=video_id)
    input_file_path = video_file.mp4_1080.path
    filename = os.path.basename(input_file_path)
...
users = users.annotate(
    rank_point=(
        Count('post__reactions', filter=Q(post__reactions__is_like=True)) - (Count('post__reactions', filter=Q(post__reactions__is_like=False)))
    )
).filter(rank_point__gt=0)
